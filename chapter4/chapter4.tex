...
%
We can study the relative impact of different datasets by studying results for the fits \#j7nw and \#j88nw, where 
either 7 TeV or 8 TeV data only are included.
By inspection of Fig.~\ref{fig:dijet_data_partial} we see how the impact of the two datasets 
...
not included in either fits, to the one given by the one where all dijets data are included.
...
Having assessed the impact on PDFs of jet and dijets dataset separately, we now compare results.

%
The effect on PDFs of the inclusion of jet and dijet in the NNPDF3.1 global dataset is qualitatively the same:
...
regarding dijets data only CMS 8 TeV data are currently available, while in the case of single-inclusive jets 8 TeV 
measurements from both ATLAS and CMS are available. The constraining power of dijets datasets 
...
The only exception is the ATLAS top rapidity distribution, which seems to be in tension with the ATLAS 8 TeV single-inclusive
jet data.

%
Finally, concerning fit quality, the quality of the two fits \#janw and \#danw is comparable, though slightly better
for the latter ($\chi^2=1.65$ vs. $\chi^2=1.88$). Also, the quality of the fit to dijets 
...
This confirms full consistency between the two datsets, with a marginal preference for dijets.
...
\begin{figure}[!t]
    ...
    \caption{Same as Fig.~\ref{fig:jet_data_total}, but now comparing the
      ...
      dijet data (\#danw) and highest theory accuracy (NNLO QCD+ EW) and
      default settings. In the gluon comparison (right) results are
