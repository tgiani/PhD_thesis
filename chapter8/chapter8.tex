...
\begin{align}
	...
	2 P_0 \, \int_{-\infty}^{\infty}\frac{dz}{4\pi}\,e^{i x \text{P}_z z}\,\mathcal{M}_A^{(0)}\lp zP_z,-z_3^2 \rp\,, 
\end{align}
which will be the case considered in this Chapter. 
As in the case of standard PDFs (which from now on we will call {\em light-cone}\ PDFs),
...
%Despite the details of the full QCD computation are of course more complicated,
%see Ref.~\cite{Ishikawa:2017faj}, as mentioned before 
%its main features and results are well described by the simple example of the previous Chapter.
...
In this Section, we describe the lattice data used in this Chapter, presenting
...
\end{figure}
%
The bare lattice data contain two types of divergences. First of all, there are
...
notations introduced above, the parton distribution $f_{3}$ is
defined as
\begin{align}
	f_{3}\lp x, \mu^2 \rp = 
	\begin{cases}
...
\label{sec:fk_qpdf}
The main point of the previous Sections is that the lattice equal-time correlators 
...
matching and the Fourier trasnform, expressing them through a single matrix
